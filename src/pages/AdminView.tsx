...
import { normalizeRunnerImageInput, resolveRunnerImageUrl } from '../lib/imageAssets';
...
function AdminContent({
    ...
    race: NonNullable<ReturnType<typeof useRace>['race']>;
    runners: Runner[];
    teamTotal: number;
    code: string;
}) {
        ...
        <div style={{ minHeight: '100vh' }}>
            <header
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, opacity: 0.6, fontFamily: 'var(--font-mono)' }}>Admin · {race.code}</div>
                </div>
            </header>
            ...
            <main style={{ maxWidth: 900, margin: '0 auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <section className="card" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}>
                    ...
                    <div style={{ fontSize: 14, marginBottom: 10 }}>A futók ezen a linken keresztül tudnak GPS-t küldeni:</div>
                    <div
                        style={{
                            padding: 12,
                            background: '#fff',
                ...
                <section className="card">
                    ...
                    <div
                        ...
                        <Stat
                            label="Tényleges rajt / Actual start"
                            value={fmtDateTime(race.actual_start_at)}
                            accent={race.actual_start_at ? 'success' : undefined}
                        />
                        <Stat
                            label="Tényleges cél / Actual end"
                            value={fmtDateTime(race.actual_end_at)}
                    ...
                    <div className="race-control-row">
                        <button className="btn btn-success" disabled={busy} onClick={() => guarded(() => api.raceStart(race.id))}>
                            <Play size={14} /> RAJT / START
                        </button>
                        <button className="btn btn-dark" disabled={busy} onClick={() => guarded(() => api.raceEnd(race.id))}>
                ...
                <section className="card">
                                ...
                                <div
                                    ...
                                    {isEditing ? (
                                        <div className="edit-runner-grid">
                                            ...
                                            />
                                            <input
                                                value={editDraft.img_url ?? ''}
                                                onChange={(e) => setEditDraft({ ...editDraft, img_url: e.target.value })}
                                                placeholder="Kép URL"
                                            />
                                            <input
                                                type="number"
                                                step="0.1"
                                                value={editDraft.target_dist ?? ''}
                                                onChange={(e) => setEditDraft({ ...editDraft, target_dist: Number(e.target.value) })}
                                                placeholder="km"
                                            />
                                        </div>
                                    ) : (
                                        <div className="runner-info">
                                            ...
                                            <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>
                                                ...
                                                {runner.is_active && (
                                                    <span style={{ color: 'var(--active)', marginLeft: 10, fontWeight: 700 }}>
                                                        ● Most fut
                                                    </span>
                                                )}
                                                {runner.is_finished && (
                                                    <span style={{ color: 'var(--finished)', marginLeft: 10, fontWeight: 700 }}>
                                                        ✓ Kész
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <>
                                                <button
                                                    ...
                                                    onClick={() =>
                                                        guarded(() =>
                                                            runner.is_active
                                                                ? api.stopRunner(runner.id)
                                                                : api.setActiveRunner(race.id, runner.id),
                                                        )
                                                    }
                                                    disabled={busy}>
                    ...
                    <div
                        ...
                        <div className="new-runner-grid">
                            ...
                            />
                            <input
                                placeholder="Kép URL (opc.) / Image URL"
                                value={newRunner.imgUrl}
                                onChange={(e) => setNewRunner({ ...newRunner, imgUrl: e.target.value })}
                            />
                            <input
                                type="number"
                                step="0.1"
                                placeholder="km"
                                value={newRunner.targetDist}
                                onChange={(e) => setNewRunner({ ...newRunner, targetDist: e.target.value })}
                            />
                        ...
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8 }}>
                            💡 Létrehozás után szerkesztéskor is bármikor módosíthatod a kép URL-jét.
                        </div>
                    </div>
                </section>
            </main>

            {gpsSetupRunner && <GPSLoggerSetup runner={gpsSetupRunner} supabaseUrl={supabaseUrl} onClose={() => setGpsSetupRunner(null)} />}
        </div>
    );
}
